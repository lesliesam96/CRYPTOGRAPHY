...
class Tetromino:
    def __init__(self, tetris, current=True, level=1):
        self.tetris = tetris
        self.shape = self.choose_shape(level)
    ...
    def choose_shape(self, level):
        if level == 1:
            return random.choice(['O', 'I', 'L'])
        elif level == 2:
            return random.choice(['J', 'T'])
        elif level == 3:
            return random.choice(['S', 'Z'])
        else:
